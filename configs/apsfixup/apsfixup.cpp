// libapsfixup.so — permanent native fix for the OnePlus APS turbo soft/green/crash bug.
// Ported from dodge (OP13/SM8750) -> infiniti (OP15/SM8850) -> aston (12R/SM8550).
//
...
//
// Loaded into com.oplus.camera as a DT_NEEDED of /odm/lib64/libAlgoProcess.so. Offsets MUST be
// adapted per platform by analyzing the ODM blobs with readelf:
//   readelf -r odm/lib64/libAlgoProcess.so | grep p010LSB2MSBNeon   # GOT slot offset
//   readelf -s odm/lib64/libAlgoProcess.so | grep p010LSB2MSBNeon   # FUNC offset
//   readelf -r odm/lib64/libAlgoInterface.so | grep "dlsym@LIBC"     # dlsym GOT slot offset
//
// TODO: Set SM8550 (12R) offsets by analyzing the OOS dump's libAlgoProcess/libAlgoInterface.
// Current values are PLACEHOLDERS from infiniti (SM8850) that WILL NOT WORK on 12R.
//
...
// TODO: SM8550 (12R) offsets — need readelf analysis of OOS dump's libAlgoProcess.so + libAlgoInterface.so
// These are PLACEHOLDERS from infiniti (SM8850) — WRONG for 12R.
static const uintptr_t P010_FUNC_OFF = 0x4fc25c;   // TODO: p010LSB2MSBNeon FUNC offset in 12R's libAlgoProcess
static const uintptr_t P010_GOT_OFF  = 0x689ba8;   // TODO: GOT JUMP_SLOT offset for p010LSB2MSBNeon in 12R's libAlgoProcess
static const uintptr_t DLSYM_GOT_OFF = 0x1bb67c8;  // TODO: dlsym@LIBC JUMP_SLOT offset in 12R's libAlgoInterface
...
// Load bias of a DT_NEEDED/dlopen'd object, via dl_iterate_phdr (runs under the linker lock -> an
// ATOMIC, consistent view; no /proc/self/maps torn read). Kept from the v1.3.1 hardening: parsing
// /proc/self/maps with fgets+sscanf while the linker is concurrently mmap'ing the camera blobs can
// return a bogus/unmapped base; dl_iterate_phdr returns dlpi_addr (true bias) keyed on the basename.
struct mb_ctx { const char* want; uint64_t base; bool found; };
static int mb_cb(struct dl_phdr_info* info, size_t, void* data) {
    auto* c = (mb_ctx*)data;
    const char* nm = info->dlpi_name;
    if (!nm || !*nm) return 0;                       // skip the main executable (empty name)
    const char* slash = strrchr(nm, '/');
    const char* basenm = slash ? slash + 1 : nm;     // compare basenames, not full paths
    if (strcmp(basenm, c->want) == 0) {
        c->base  = (uint64_t)info->dlpi_addr;        // load bias (== ELF base for a PIE/.so)
        c->found = true;
...
static bool module_base(const char* name, uint64_t* out_base) {
    mb_ctx c{ name, 0, false };
    dl_iterate_phdr(mb_cb, &c);
    if (c.found) { *out_base = c.base; return true; }
    return false;
}
...
// ---- install (dodge-faithful: unconditional GOT redirect, no BuildId gate) ----
...
static void try_install() {
    ...
    if (!g_p010_done && module_base("libAlgoProcess.so", &base)) {
        ...
        if (got_redirect(base + P010_GOT_OFF, (void*)wrap_p010, &old)) {
            ...
            if (old != expect) LOGW("GOT[p010]=%p expected %p (blob drift / .201<->.300 -- harmless)", old, expect);
